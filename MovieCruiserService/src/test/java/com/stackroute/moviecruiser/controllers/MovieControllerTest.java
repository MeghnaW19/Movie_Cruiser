...
public class MovieControllerTest {
	...
	public void testSaveNewMovie() throws Exception {
		...
		movieMockMvc.perform(post("/api/v1/movieservice/movie").header("authorization", "Bearer " + token).contentType(MediaType.APPLICATION_JSON).content(asJsonString(movie)))
	...
	public void testGetMyMovies() throws Exception {
		String token = "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMDMiLCJpYXQiOjE1MjQ0Nzg2MTN9.DRTv6IfxMuYr2EzJo3HOOfsXPOzL6MAy-B2XwkeGaDI";
		when(movieService.getMyMovies("103")).thenReturn(movies);
		movieMockMvc.perform(get("/api/v1/movieservice/movies").header("authorization", "Bearer " + token)
				.contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andDo(print());
		verify(movieService, times(1)).getMyMovies("103");
		verifyNoMoreInteractions(movieService);
	}
