...
public class SwaggerConfig {
	...
	public Docket productApi() {
		return new Docket(DocumentationType.SWAGGER_2).select()
				.apis(RequestHandlerSelectors.basePackage("com.stackroute.auth")).paths(regex("/user.*")).build();
		// .apiInfo(metaInfo());
	}

	public ApiInfo metaInfo() {
		ApiInfo apiInfo = new ApiInfo("Authentication using JWT", "Spring Boot REST API for authentication", "1.0",
				"Terms of service",
